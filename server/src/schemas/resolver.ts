...
export interface Context {
  ...
  export const resolvers: IResolvers = {
    ...
    Mutation: {
      ...
      login: async (_, { email, password }: { email: string; password: string }): Promise<AuthPayload> => {
      ...
      updateUser: async (_, { id, input }: { id: string; input: UpdateUserInput }, { user }: Context) => {
        if (!user) throw new AuthenticationError('Not authenticated');
      ...
      // Dog mutations
      createDog: async (_, { input }: { input: CreateDogInput }, { user }: Context) => {
        if (!user) throw new AuthenticationError('Not authenticated');
      ...
      updateDog: async (_, { id, input }: { id: string; input: UpdateDogInput }, { user }: Context) => {
        if (!user) throw new AuthenticationError('Not authenticated');
        
        const dog = await Dog.findById(id);
        if (!dog) throw new UserInputError('Dog not found');
        if (dog.owner.toString() !== user.id) throw new AuthenticationError('Not authorized');
  
        return await Dog.findByIdAndUpdate(id, input, { new: true }).populate('owner');
      },
      ...
      updateMatchStatus: async (
        _, 
        { matchId, dogId, status }: { matchId: string; dogId: string; status: MatchStatus }
      ) => {
